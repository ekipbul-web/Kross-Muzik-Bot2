...
client.on('messageCreate', async (message) => {
    ...
    if (command === 'oynat' || command === 'play') {
        ...
        try {
            // Şarkı arama motoru
            let search_info = await play.search(searchKeyword, { limit: 1 });
            if (search_info.length === 0) return message.reply('❌ Şarkı bulunamadı.');

            // Çalmama problemini çözen canlı akış uyumluluk modu
            ...
            const embed = new EmbedBuilder()
                ...
                .setDescription(`**[${search_info[0].title}](${search_info[0].url})**`)
                .setThumbnail(search_info[0].thumbnails?.[0]?.url || '')
            ...
            console.error(error);
            message.channel.send('❌ Ses akışı başlatılırken bir hata oluştu.');
        }
    }
});
